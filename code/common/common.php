  ...
  function sanitize($before)
  {
    if (is_array($before)) {
      ...
      foreach ($before as $key => $value) {
        if (is_array($value)) {
          $after[$key] = array_map(function($v) {
            return is_string($v) ? htmlspecialchars($v, ENT_QUOTES, 'UTF-8') : $v;
          }, $value);
        } else {
          $after[$key] = is_string($value) ? htmlspecialchars($value, ENT_QUOTES, 'UTF-8') : $value;
        }
      }
      return $after;
    } else {
      return is_string($before) ? htmlspecialchars($before, ENT_QUOTES, 'UTF-8') : $before;
    }
  }
